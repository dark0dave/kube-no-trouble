...
[latest release](https://github.com/dark0dave/kube-no-trouble/releases/latest)
...
`ghcr.io/dark0dave/kube-no-trouble:latest` (also available tagged with each
...
$ docker run -it --rm \
    ...
    ghcr.io/dark0dave/kube-no-trouble:latest \
    -k /.kubeconfig
```
...
$ kubectl run kubent --restart=Never --rm -i --tty \
    --image ghcr.io/dark0dave/kube-no-trouble:latest \
...
git clone https://github.com/dark0dave/kube-no-trouble.git
cd kube-no-trouble/
# Build
...
Please open any issues and/or PRs against github.com/dark0dave/kube-no-trouble repository. See our [contribution guide](.github/CONTRIBUTING.md) for more details
